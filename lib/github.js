...
const AUTH_TOKEN = config.get('githubAuthToken');
...
exports.getRepo = async (orgName, repoName, minCacheTime) => {
  ...
  const opts = {};
  if (AUTH_TOKEN && __validateAuthToken(AUTH_TOKEN)) {
    opts.headers = {Authorization: 'token ' + AUTH_TOKEN};
  }

  const {data} = await axios.get(url, opts).catch(err => {
    console.error('Failed fetching URL: ', url);
    if (err.response && err.response.status === 403) {
      return Promise.reject(
        new Error(
          "You're rate-limited. Specify an auth token? (prcheck setConfig githubAuthToken <YOUR_TOKEN>)"
        )
      );
    }
